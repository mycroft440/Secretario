#!/usr/bin/env bash
set -euo pipefail

PJ_TAG="2.17"
ROOT_DIR="$(pwd)"
WORK_DIR="$ROOT_DIR/.third_party/pjproject"
OUT_DIR="$ROOT_DIR/pjsua2/generated"
NDK_VERSION="28.2.13676358"

ANDROID_NDK_ROOT="${ANDROID_NDK_ROOT:-${ANDROID_HOME:?ANDROID_HOME is required}/ndk/$NDK_VERSION}"
export ANDROID_NDK_ROOT

if [[ ! -d "$ANDROID_NDK_ROOT" ]]; then
  echo "Android NDK not found at $ANDROID_NDK_ROOT" >&2
  exit 1
fi

rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR/java" "$OUT_DIR/jniLibs"

if [[ ! -d "$WORK_DIR/.git" ]]; then
  rm -rf "$WORK_DIR"
  mkdir -p "$(dirname "$WORK_DIR")"
  git clone --depth 1 --branch "$PJ_TAG" https://github.com/pjsip/pjproject.git "$WORK_DIR"
fi

cat > "$WORK_DIR/pjlib/include/pj/config_site.h" <<'EOF'
#define PJ_CONFIG_ANDROID 1
#define PJMEDIA_HAS_VIDEO 0
#include <pj/config_site_sample.h>
EOF

copy_cxx_runtime() {
  local abi="$1"
  local triple
  case "$abi" in
    arm64-v8a) triple="aarch64-linux-android" ;;
    armeabi-v7a) triple="arm-linux-androideabi" ;;
    x86_64) triple="x86_64-linux-android" ;;
    *) return 0 ;;
  esac

  local cxx="$ANDROID_NDK_ROOT/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/$triple/libc++_shared.so"
  if [[ -f "$cxx" ]]; then
    cp "$cxx" "$OUT_DIR/jniLibs/$abi/libc++_shared.so"
  fi
}

for ABI in arm64-v8a armeabi-v7a x86_64; do
  echo "==> Building PJSIP $PJ_TAG for $ABI"
  cd "$WORK_DIR"
  make distclean >/dev/null 2>&1 || true

  TARGET_ABI="$ABI" ./configure-android --use-ndk-cflags
  make dep
  make -j2

  cd "$WORK_DIR/pjsip-apps/src/swig"
  make clean >/dev/null 2>&1 || true
  make -j2

  GENERATED_JAVA="$WORK_DIR/pjsip-apps/src/swig/java/android/app/src/main/java/org/pjsip/pjsua2"
  GENERATED_SO="$WORK_DIR/pjsip-apps/src/swig/java/android/app/src/main/jniLibs/armeabi/libpjsua2.so"

  test -d "$GENERATED_JAVA"
  test -f "$GENERATED_SO"

  if [[ ! -d "$OUT_DIR/java/org/pjsip/pjsua2" ]]; then
    mkdir -p "$OUT_DIR/java/org/pjsip"
    cp -R "$GENERATED_JAVA" "$OUT_DIR/java/org/pjsip/"
  fi

  mkdir -p "$OUT_DIR/jniLibs/$ABI"
  cp "$GENERATED_SO" "$OUT_DIR/jniLibs/$ABI/libpjsua2.so"
  copy_cxx_runtime "$ABI"
done

cd "$ROOT_DIR"
COUNT_JAVA="$(find "$OUT_DIR/java/org/pjsip/pjsua2" -type f -name '*.java' | wc -l | tr -d ' ')"
COUNT_SO="$(find "$OUT_DIR/jniLibs" -type f -name 'libpjsua2.so' | wc -l | tr -d ' ')"

echo "PJSUA2 prepared: $COUNT_JAVA Java bindings, $COUNT_SO native ABIs"
test "$COUNT_JAVA" -gt 100
test "$COUNT_SO" -eq 3
